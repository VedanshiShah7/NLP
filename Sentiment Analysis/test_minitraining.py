...
class TestTextClassifyBaselineMiniTrain(unittest.TestCase):
    ...
    def test_GenerateTuplesFromTrainingFile(self):
        ...
        actualExamples = [('ID-2', 'The hotel was not liked by me', '0'), ('ID-3', 'I loved the hotel',
                                                                           '1'), ('ID-1', 'The hotel was great', '1'), ('ID-4', 'I hated the hotel', '0')]
    ...
    def test_precisionrecallf1together(self):
        ...
        self.assertAlmostEqual(
            p, tc_model.precision(gold, classified), places=4)
        self.assertAlmostEqual(r, tc_model.recall(gold, classified), places=4)
        self.assertAlmostEqual((2 * p * r) / (p + r),
                               tc_model.f1(gold, classified), places=4)
    ...
    def test_ScorePositiveExample(self):
        ...
        pos = ((1+1)/(8+12)) * ((1+1)/(8+12)) * \
            ((1+1)/(8+12)) * ((2+1)/(8+12)) * (2/4)

        neg = ((1+1)/(11+12)) * ((0+1)/(11+12)) * \
            ((1+1)/(11+12)) * ((2+1)/(11+12))*(2/4)
        actualScoreDistribution = {'1': pos, '0': neg}

        self.assertAlmostEqual(
            actualScoreDistribution['0'], score['0'], places=5)
        self.assertAlmostEqual(
            actualScoreDistribution['1'], score['1'], places=5)
    ...
    def test_ScorePositiveExampleRepeats(self):
        ...
        # P(C|text)=P(I|C)*P(loved|C)*P(the|C)*P(hotel|C),where C is either 0 or 1(Classifier)
        pos = ((1+1)/(8+12))*((1+1)/(8+12))*((1+1)/(8+12)) * \
            ((2+1)/(8+12))*((1+1)/(8+12))*((1+1)/(8+12))*((2+1)/(8+12))*(2/4)
        neg = ((1+1)/(11+12))*((0+1)/(11+12))*((1+1)/(11+12))*((2+1) /
                                                               (11+12))*((0+1)/(11+12))*((1+1)/(11+12))*((2+1)/(11+12))*(2/4)
        actualScoreDistribution = {'1': pos, '0': neg}
        self.assertAlmostEqual(
            actualScoreDistribution['0'], score['0'], places=5)
        self.assertAlmostEqual(
