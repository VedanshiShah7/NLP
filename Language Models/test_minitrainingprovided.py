import unittest
import lm_friends as lm_utils
from lm_friends import LanguageModel

# Uses the following training files
# iamsam.txt
# iamsam2.txt
# unknowns_mixed.txt
# unknowns.txt
...
class TestMiniTraining(unittest.TestCase):
    ...
    def test_unigramlaplace(self):
        ...
        sentences = lm_utils.read_sentences("training_files/iamsam.txt")
        lm.train(sentences)
        # ((2 + 1) / (10 + 5))
        self.assertAlmostEqual(.2, lm.score(
            "<s>"), msg="tests probability of <s>, trained on iamsam.txt")
        # ((2 + 1) / (10 + 5)) ** 2
        self.assertAlmostEqual(.04, lm.score(
            "<s> </s>"), msg="tests probability of <s> </s>, trained on iamsam.txt")
        # ((2 + 1) / (10 + 5)) ** 3
        self.assertAlmostEqual(.008, lm.score(
            "<s> i </s>"), msg="tests probability of <s> i </s>, trained on iamsam.txt")

    def test_unigram(self):
        lm = LanguageModel(1, False, line_begin="<s>", line_end="</s>")
        sentences = lm_utils.read_sentences("training_files/iamsam2.txt")
        lm.train(sentences)
        # ((4) / (20))
        self.assertAlmostEqual(.2, lm.score(
            "<s>"), msg="tests probability of <s>, trained on iamsam2.txt")
        # (2 / 20)
        self.assertAlmostEqual(.1, lm.score(
            "sam"), msg="tests probability of sam, trained on iamsam2.txt")
        # (4 / 20) * (2 / 20) * (4 / 20)
        self.assertAlmostEqual(.004, lm.score(
            "<s> ham </s>"), msg="tests probability of <s> ham </s>, trained on iamsam2.txt")

    def test_unigramunknowns(self):
        lm = LanguageModel(1, False, line_begin="<s>", line_end="</s>")
        sentences = lm_utils.read_sentences(
            "training_files/unknowns_mixed.txt")
        lm.train(sentences)
        # ((1) / (11))
        self.assertAlmostEqual(1 / 11, lm.score("flamingo"), places=3,
    ...
    def test_bigramunknowns(self):
        ...
        lm.train(sentences)
        # ((0) / (2))
        self.assertEqual(0, lm.score(
            "<s> flamingo"), msg="tests probability of <s> flamingo, trained on unknowns_mixed.txt")
    ...
    def test_bigramunknownslaplace(self):
        ...
        lm.train(sentences)
        # (0 + 1) / (2 + 6)
        self.assertAlmostEqual(1 / 8, lm.score("<s> flamingo"), places=3,
    ...
    def test_bigram(self):
        ...
        lm.train(sentences)
        # (2) / (4)
        self.assertAlmostEqual(.5, lm.score(
            "<s> i"), msg="tests probability of <s> i, trained on iamsam2.txt")
        # (2 / 4) * (4 / 4) * (2 / 4)
        self.assertAlmostEqual(.25, lm.score(
            "<s> i am </s>"), msg="tests probability of <s> i am </s>, trained on iamsam2.txt")
    ...
    def test_bigramlaplace(self):
        ...
        lm.train(sentences)
        # (2 + 1) / (4 + 6)
        self.assertAlmostEqual(.3, lm.score(
            "<s> i"), msg="tests probability of <s> i, trained on iamsam2.txt")
        # ((2 + 1) / (4 + 6)) * ((4 + 1) / (4 + 6)) * ((2 + 1) / (4 + 6))
        self.assertAlmostEqual(.045, lm.score(
            "<s> i am </s>"), msg="tests probability of <s> i am </s>, trained on iamsam2.txt")
    ...
    def test_generatebigramconcludes(self):
        ...
        self.assertEqual(2, len(
            sents), msg="tests that you generated 2 sentences and that generate concluded")
        print(sents)
    ...
    def test_onlyunknownsgenerationandscoring(self):
        lm = LanguageModel(1, True, line_begin="<s>", line_end="</s>")
        sentences = lm_utils.read_sentences("training_files/unknowns.txt")
        lm.train(sentences)
